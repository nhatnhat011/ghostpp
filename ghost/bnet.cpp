...
bool CBNET :: Update( void *fd, void *send_fd )
{
	...
	if( !m_Socket->GetConnecting( ) && !m_Socket->GetConnected( ) && ( m_FirstConnect || GetTime( ) - m_LastDisconnectedTime >= 90 ) )
	{
		...
		CONSOLE_Print( "[BNET: " + m_ServerAlias + "] connecting to server [" + m_Server + "]" );
		...
		if( m_ServerIP.empty( ) )
		{
			m_Socket->Connect( m_GHost->m_BindAddress, m_Server, m_ServerPort );
		...
		else
		{
			...
			m_Socket->Connect( m_GHost->m_BindAddress, m_ServerIP, m_ServerPort );
