...
CGHost :: CGHost( CConfig *CFG )
{
	...
	for( uint32_t i = 1; i < 10; ++i )
	{
		...
		int ServerPort = CFG->GetInt( Prefix + "serverport", 6112 );
		...
		m_BNETs.push_back( new CBNET( this, Server, ServerAlias, ServerPort, BNLSServer, (uint16_t)BNLSPort, (uint32_t)BNLSWardenCookie, CDKeyROC, CDKeyTFT, CountryAbbrev, Country, LocaleID, UserName, UserPassword, FirstChannel, RootAdmin, BNETCommandTrigger[0], HoldFriends, HoldClan, PublicCommands, War3Version, EXEVersion, EXEVersionHash, PasswordHashType, PVPGNRealmName, MaxMessageLength, i ) );
	}
